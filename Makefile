...
#---------------------------------------------------------------------------------
# Build all sources and link the final ELF.  The recursive make call
# passes $(OUTPUT).elf as the target so that the link rule in the
# secondary phase is invoked.  Without this, running `make` only
# compiles object files but never produces the ELF.
...
clean:
	@echo clean ...
	@rm -fr $(BUILD) $(TARGET).3dsx $(OUTPUT).smdh $(TARGET).elf $(GFXBUILD) $(TARGET).cia $(TARGET).cxi
...
#---------------------------------------------------------------------------------
# Auto‑generate the Home‑Menu icon (SMDH) from icon.png
#---------------------------------------------------------------------------------
$(OUTPUT).smdh : $(ICON_PNG)
	@echo "  [SMDH] $@"
